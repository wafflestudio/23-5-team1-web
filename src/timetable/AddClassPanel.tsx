...
import { buildTimeOptions, snapToStep, STEP_MIN } from "./time";
...
export function AddClassPanel({ onAdd }: Props) {
    ...
    return (
        <aside className="tt-panel">
            ...
            <label>
                <div>과목명 (필수)</div>
                <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="경제학개론" />
            </label>

            <label>
                <div>교수명 (선택)</div>
                <input value={professor} onChange={(e) => setProfessor(e.target.value)} placeholder="박이택"/>
            </label>
            ...
            <div>
                ...
                <button className="tt-link" type="button" onClick={() => alert("+ 시간 추가")}>
                    + 시간 추가
                </button>

                {!valid && (
                    <div className="tt-error"> 시간 범위가 잘못되었습니다. (종료가 시작보다 늦어야 하고 5분 단위여야 합니다. )</div>
                )}
            </div>
            <button  className="tt-save" type="button" disabled={!valid} onClick={handleSave}>
                저장
            </button>
        </aside>
    )
