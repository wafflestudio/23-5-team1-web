import type { NumberLiteralType, StringLiteral } from "typescript";
import type { Day, Course, TimeSlot } from "../util/types";
...
export function durationToHeight(startMin: number, endMin: number, cfg: GridConfig) {
    return (endMin - startMin) * cfg.ppm;
}

export function flattenToBlocks(courses: Course[], cfg: GridConfig): PositionedBlock[] {
...
export function hasOverlap(existing: TimeSlot[], next: TimeSlot) {
    return existing.some((s) => s.day === next.day && next.startMin < s.endMin && s.startMin < next.endMin);
}