...
const MemoPageCard = ({ memo, onDelete }: { memo: Memo; onDelete: Dispatch<SetStateAction<number | null>> }) => {
    const [editMode, setEditMode] = useState<boolean>(false);
    const [content, setContent] = useState<string>(memo.content);
    const [tagNames, setTagNames] = useState<string[]>(memo.tags.map(m => m.name));
        ...
        <div className={styles.cardContainer}>
            ...
            <div className={styles.cardWrapper}>
                ...
                <div className={styles.tagsContainer}>
                    ...
                    {editMode && (
                        <button type='button' className={styles.addTagBtn} onClick={handleNewTag}>
                    ...
                    <FaTrashCan onClick={() => onDelete(memo.id)} className={styles.deleteBtn} size={22} color="#7c7c7c" />
                    <HiOutlinePencilAlt onClick={()=>setEditMode(true)} className={styles.editIcon} size={25} color="#7c7c7c"  />
                    </div>
                    : <IoMdDoneAll onClick={handleSave} className={styles.checkIcon} size={25} color="#7c7c7c" />}
                </div>
            </div>
    )
}
