...
const DetailMemo = ({ eventId, isMemoExpanded, setIsMemoExpanded }: DetailMemoProps) => {
	const { eventMemos, addMemo, editMemoTag, editMemoContent } = useUserData();
    // events that user has written memos
    const CURRENT_MEMO: Memo | undefined = eventMemos.find(m => m.eventId === eventId);

    // 메모 관련 상태
    const [memoContent, setMemoContent] = useState<string>(CURRENT_MEMO ? CURRENT_MEMO.content : "");
    const [isSavingMemo, setIsSavingMemo] = useState<boolean>(false);

    // 태그 관련 상태
    const [tagNames, setTagNames] = useState<string[]>(CURRENT_MEMO ? CURRENT_MEMO.tags.map(t => t.name) : []);
    ...
    const currentTagNames = CURRENT_MEMO ? CURRENT_MEMO.tags.map(t => t.name) : [];
    ...
    return (
        <div className={`${styles.memoContent} ${isMemoExpanded ? styles.active : ''}`}>
				{/* 헤더 라인 (연필/메모하기/저장하기) */}
